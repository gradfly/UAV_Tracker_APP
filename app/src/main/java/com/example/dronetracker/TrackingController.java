...
//    public static final float FOV_VERTICAL = 80.0f;//fpv摄像头竖直视角
    ...
    public TrackingController() {
        ...
        throttleController = new PIDController(0.3f, 0.000f, 0.05f);//ki=0.001, 映射50度到5.0输出
        pitchController = new PIDController(0.3f, 0.00f, 0.1f);//ki=0.01
        // 初始化默认距离，对应进度条中值50
    ...
    public TrackingResult processTarget(Rect targetRect, float deltaTime) {
        ...
        lastPitch = lastPitch + clamp(ALPHA * (pitchOutput - lastPitch), -3f, 3f);

        DroneCommand command = new DroneCommand(lastYaw, lastThrottle, lastPitch, 0, estimatedDistance);
